...
Landed in self-evolve batch 4 (A-tier baseline-relative ACCEPT, e=129.27, +12, 0 regressions):
these were xfail headroom, the additive fix in digest.py flipped them to XPASS, and the markers
are now removed so they stand as permanent regression guards for the overslept-catch-up axis.
"""
import digest as dg
...
def test_capability_present():
    assert hasattr(dg, "missed_digest_dates") and hasattr(dg, "catch_up_digests")
